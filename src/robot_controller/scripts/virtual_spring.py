...
class Spring:
    def __init__(self):
        ...
        self.ur3_position = None
        self.force_feedback_status = False
        rospy.loginfo("Force feedback status: " + str(self.force_feedback_status))

        # Spring constant (tunable parameter)
        self.k_spring = 25  # Adjust this value for desired stiffness
        self.max_force = 0.75 # Maximum force to apply
        ...
        #Subscriber for UR3 joint states
        rospy.Subscriber("/ur3/joint_states",JointState, self.ur3_joint_callback, queue_size=1)
    ...
    def ur3_joint_callback(self, data):
        self.ur3_position = data.position
        # rospy.loginfo(f"UR3 position: {self.ur3_position}")
    ...
    def omni_joint_callback(self, data):
        ...
        if self.ur3_position is not None and self.force_feedback_status:
            rospy.loginfo("Applying spring force")
    ...
    def apply_spring_force(self):
        ...
        ur3_joint_position = self.calculate_forward_kinematics(self.ur3_position)
        ...
        dx = real_omni_position.x - ur3_joint_position.x
        dy = real_omni_position.y - ur3_joint_position.y 
        dz = real_omni_position.z - ur3_joint_position.z 
